...
public class DemoButtonsMultimage extends JFrame
{
	/**
	 * 
	 */
	private static final long serialVersionUID = 8308048350690502109L;
	...
	static class SignatureDialog extends JDialog implements ITabletHandler
	{
		 ...
		 * 
		 */
		private static final long serialVersionUID = 4820345269259625428L;
		...
		public SignatureDialog(Frame frame, UsbDevice usbDevice, TlsDevice tlsDevice, boolean useSigMode, boolean bulk, Encodings encodingOpc, int occ) throws STUException
		{
			super(frame, true);
			this.bulk = bulk;
			this.encodingOpc = encodingOpc;
			this.occ = occ;
			...
			try
			{
				...
				if ( this.encodingOpc == Encodings.E24B || ( this.encodingOpc == Encodings.DEF && ((encodingFlag & EncodingFlag.EncodingFlag_24bit) != 0)))
				{
					this.encodingMode = (this.bulk && this.tablet.supportsWrite()) ? EncodingMode.EncodingMode_24bit_Bulk : EncodingMode.EncodingMode_24bit;
				}
				else if ( this.encodingOpc == Encodings.E16B || ( this.encodingOpc == Encodings.DEF && ((encodingFlag & EncodingFlag.EncodingFlag_16bit) != 0)))
				{
					this.encodingMode = this.bulk && this.tablet.supportsWrite() ? EncodingMode.EncodingMode_16bit_Bulk : EncodingMode.EncodingMode_16bit;
				}
				else if (this.encodingOpc == Encodings.E1BZ)
				{
				...
				{
					System.out.println("El dispositivo soporta encripación. Inciliza el cifrado.");
					this.tablet.startCapture(0xc0ffee);
					encrypted = true;
				}else {
					System.out.println("El dispositivo no soporta encripación");
				}
		...
		private Color[] colors = {Color.black, Color.white, Color.BLUE, Color.red, Color.YELLOW,
		                          Color.black.brighter(), Color.white.cyan, Color.BLUE.brighter(), Color.red.brighter(), Color.YELLOW.brighter()};
		private void showImges() throws STUException {
			if (this.useSigMode || this.occ <= 0)
				return;

			for(int i=1; i<this.occ+1;i++ ) {
				
				Color c = colors[i%colors.length];
				BufferedImage bi = getChessboard(800, 480, c);
				Graphics2D g = bi.createGraphics();
				
				g.setColor(Color.orange);
				g.setFont(new Font("Arial", Font.PLAIN, 200));
				g.drawString(Integer.toString(i), 400, 200);
				
				byte[] buf = ProtocolHelper.flatten(bi, bi.getWidth(), bi.getHeight(), encodingMode);
			
				this.tablet.writeImage(this.encodingMode, buf);

			}
		}

		public static void drawBackground(Graphics2D g, int width, int height, Color c) {
...
//				for(int y=x%2; y<yTop; y+=2) {
		...
		public static BufferedImage getChessboard(int w, int h, Color c) {
			BufferedImage bi = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
			drawBackground(bi.createGraphics(), w, h, c);
			return bi;
		}
	...
	private void onGetSignature()
	{
		try
		{
			...
			if (usbDevice != null || tlsDevice != null)
			{
				...
				SignatureDialog signatureDialog = new SignatureDialog(this, usbDevice,tlsDevice, sigMode, this.bulk, this.encodingOpc, this.occ);
	...
	public DemoButtonsMultimage(boolean bulk, Encodings encodingOpc, int occ)
	{
		this.bulk = bulk;
		this.encodingOpc = encodingOpc;
		this.occ = occ;
	...
	private static void runProgram(boolean bulk, Encodings encodingOpc, int occ)
	{
		DemoButtonsMultimage sample = new DemoButtonsMultimage(bulk, encodingOpc, occ);
		sample.setVisible(true);
	}

	enum Encodings{
		DEF, E1BZ, E1B, E16B, E24B;
	}
	...
	public static void main(String[] args){
		...
		boolean bulk;
		if (args.length > 0)
			bulk = Boolean.parseBoolean(args[0]);
		else
			bulk = true;
		
		Encodings encodingOpc;
		if (args.length > 0)
			encodingOpc = Encodings.valueOf(args[1]);
		else
			encodingOpc = Encodings.DEF;
		...
		EventQueue.invokeLater(new Runnable()
		{
			public void run()
			{
				runProgram(bulk, encodingOpc, occ);
			}
		});
	}
}
